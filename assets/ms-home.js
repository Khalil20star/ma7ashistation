(() => {
  ...
  const railSelector = [
    ...
    '.ms-best__viewport',
    '.ms-featured__products-viewport',
  ...
  const nativeMobileSelector = '.ms-best__viewport, .ms-featured__products-viewport, .ms-offers__viewport, .ms-videos__viewport';
  ...
  function bindTouchRail(rail) {
    ...
    rail.addEventListener('touchstart', (event) => {
      if (window.matchMedia('(max-width: 767px)').matches && rail.matches(nativeMobileSelector)) return;
