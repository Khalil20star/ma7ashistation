...
function fixture(width, count) {
  ...
  return { component, cards, viewport };
}

for (const width of [320, 375, 390, 767, 768, 899, 900, 1199, 1200, 1440]) {
...
test('resizing clears mobile preview accessibility state', () => {
...
test('switching collections resets pagination and does not expose the previous panel', () => {
  const { component } = fixture(375, 12);
  ...
  assert.equal(nextCards.filter(card => !card.hidden && !card.inert).length, 2);
  assert.equal(component.pageButtons.filter(button => !button.hidden).length, 3);
});
